...
\toprule
\textbf{Two-Qubit Gate ($U$)} & \textbf{Coordinates $(c_1, c_2, c_3)$} & \textbf{$g_1$} & \textbf{$g_2$} & \textbf{$g_3$} & \textbf{EPR Pairs} \\
\midrule
Identity ($\mathbb{I} \otimes \mathbb{I}$) & $(0, 0, 0)$ & 1 & 0 & 3 & 0 \\
CNOT / CZ & $(\pi/4, 0, 0)$ & 0 & 0 & 1 & 1 \\
Controlled Phase $CP(\theta)$ & $(\theta/2, 0, 0)$ & $\cos^2(\theta)$ & 0 & $1 + 2\cos(\theta)$ & 1 \\
iSWAP & $(\pi/4, \pi/4, 0)$ & 0 & 0 & $-1$ & 2 \\
$\sqrt{\text{iSWAP}}$ & $(\pi/8, \pi/8, 0)$ & 0 & 0 & 0 & 2 \\
$B$-Gate & $(\pi/4, \pi/8, 0)$ & 0 & 0 & 0 & 2 \\
Double-Excitation $U(\theta)$ & $(\theta/2, \theta/2, 0)$ & $\cos^2(\theta)$ & 0 & $2\cos(2\theta)-1$ & 2 \\
SWAP & $(\pi/4, \pi/4, \pi/4)$ & $-1$ & 0 & $-3$ & 2 \\
Arbitrary $U \in \text{SU}(4)$ & $(c_1, c_2, c_3)$ & $g_1 \in [-1, 1]$ & $g_2 \in [-1, 1]$ & $g_3 \in [-3, 3]$ & $\le 3$ \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
\textbf{Synthesis Protocol} & \textbf{EPR Pairs} & \textbf{Classical Bits} & \textbf{$T$-Count} & \textbf{Network Rounds} & \textbf{Primary Advantage} \\
\midrule
\textbf{Naive Telegate CCX} & 6 & 12 & 7 & 6 & No ancilla required \\
\textbf{Jones Cat-GHZ CCX} & \textbf{3} & 6 & 4 & \textbf{2} & Minimal EPR consumption \\
\textbf{Gottesman-Chuang Telemetry} & 4 & 8 & \textbf{1} (Magic) & 3 & Fault-tolerant distillation compatible \\
