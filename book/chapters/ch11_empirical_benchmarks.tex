...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{X c c c c}
        \toprule
        \textbf{Network Topology} & \textbf{Network Diameter} & \textbf{Node Degree} & \textbf{Bisection Bandwidth} & \textbf{Mean Ebit Hop Count} \\
        \midrule
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{X c c c c}
        \toprule
        \textbf{Benchmark Algorithm} & \textbf{Config A (Naive)} & \textbf{Config B (METIS)} & \textbf{Config C (+JIT)} & \textbf{Config D (Full DQC)} \\
        \midrule
...
\begin{enumerate}[label=\textbf{11.\arabic*.}]
    ...
    \item \textbf{Cross-Entropy Benchmarking (XEB).} Prove that for the Porter-Thomas distribution $P(p) = 2^n e^{-2^n p}$, the second moment is $\mathbb{E}[p^2] = \frac{2}{2^{2n}}$ by evaluating the gamma function integral $\int_0^\infty p^2 e^{-2^n p} dp$.
