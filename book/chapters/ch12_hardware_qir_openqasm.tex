...
\begin{enumerate}
    \item \texttt{\_\_dqc\_\_rt\_\_epr\_alloc(i32 \%src\_qpu, i32 \%dst\_qpu) -> \%EPRHandle*}: Non-blocking request to allocate an entangled pair.
    \item \texttt{\_\_dqc\_\_rt\_\_teleport\_send(i32 \%dst\_qpu, \%Qubit* \%q\_ctrl, \%EPRHandle* \%epr) -> i1}: Executes local CNOT into EPR ancilla, measures, and initiates RDMA transmission of measurement bit $m_1$.
    \item \texttt{\_\_dqc\_\_rt\_\_teleport\_recv(i32 \%src\_qpu, \%Qubit* \%q\_tgt, \%EPRHandle* \%epr)}: Awaits measurement packet from sending QPU and applies conditional $\sigma_x^{m_1}$ correction.
\end{enumerate}
